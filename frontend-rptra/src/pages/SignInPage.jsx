...
const SignInPage = ({ onLogin }) => {
  ...
  const handleSignInClick = (e) => {
    ...
    onLogin({
      name: "RPTRA User",
      email: "user@rptra.id",
      avatar: "/images/galeri-1.jpg"
    });

    // 2. KEMBALI KE HOME (Sesuai permintaan terbaru)
    navigate('/'); 
  };
    ...
    <div className="min-h-screen bg-[#F8F9FD] flex flex-col justify-center items-center px-4 relative text-sm">
      ...
      <div className="w-full max-w-md bg-transparent">
        ...
        <form className="space-y-4" onSubmit={handleSignInClick}>
          <div>
            <input type="email" placeholder="Masukan Email Anda" className="w-full px-4 py-3 rounded-lg bg-gray-100 border border-gray-200 text-xs focus:outline-none focus:ring-2 focus:ring-rptra-blue font-poppins placeholder-gray-400 transition" required />
          </div>
          <div className="relative">
            <input type={showPassword ? "text" : "password"} placeholder="Masukan Kata Sandi" className="w-full px-4 py-3 rounded-lg bg-gray-100 border border-gray-200 text-xs focus:outline-none focus:ring-2 focus:ring-rptra-blue font-poppins placeholder-gray-400 transition pr-10" required />
          ...
          <div className="flex items-center justify-between mt-5">
            <button type="submit" className="bg-blue-500 hover:bg-blue-600 text-white font-bold py-2.5 px-8 rounded-lg text-xs transition shadow-md uppercase tracking-wide hover:shadow-lg transform hover:-translate-y-0.5">Masuk</button>
          ...
          <div className="mt-8 text-center">
            <Link to="/daftar"><button type="button" className="bg-blue-500 hover:bg-blue-600 text-white font-bold py-2.5 px-8 rounded-lg text-xs transition shadow-md uppercase tracking-wide w-32 mb-3 hover:shadow-lg transform hover:-translate-y-0.5">Daftar</button></Link>
            <p className="text-gray-600 text-[10px] md:text-xs font-poppins max-w-sm mx-auto leading-relaxed">Daftar disini jika anda belum mendaftar agar bisa menikmati layanan yang sudah disediakan</p>
          </div>
        </form>
      </div>
    </div>
