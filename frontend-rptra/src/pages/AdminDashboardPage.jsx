        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
            ...
            <div className="space-y-4">
                ...
                <div className="bg-[#F7F8F9] border border-[#DADADA] rounded-[10px] h-[60px] flex items-center px-6 gap-3 group hover:border-[#008C9E] transition-colors">
                    ...
                    {phoneNumber !== "-" && (
                         <button onClick={() => {navigator.clipboard.writeText(phoneNumber); alert("Nomor disalin!")}} className="text-gray-400 hover:text-black" title="Salin Nomor">
...
const AdminDashboardPage = ({ user, onLogout }) => {
  ...
  // === TAB 1: DASHBOARD (UPDATE: MENAMPILKAN AKTIVITAS TERAKHIR) ===
  const TabDashboard = () => {
                    ...
                    {loading ? (
                        ...
                        filteredData.map((item, index) => (
                            <tr key={item.id} className={`font-poppins text-sm text-gray-800 ${index % 2 === 0 ? 'bg-[#89C9D1]' : 'bg-[#EBF7F8]'}`}>
                                <td className="p-4 flex items-center gap-3 cursor-pointer group" onClick={() => setSelectedBooking(item)}>
                                ...
                                <td className="p-4 border-l border-white/20">
                                    <div className="flex justify-center gap-3">
                                        <button onClick={() => updateBookingStatus(item.id, 'reject')} className="w-8 h-8 rounded-full bg-red-500 text-white flex items-center justify-center hover:bg-red-600 transition shadow-md"><svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}><path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12" /></svg></button>
                                        <button onClick={() => updateBookingStatus(item.id, 'approve')} className="w-8 h-8 rounded-full bg-[#6CC24A] text-white flex items-center justify-center hover:bg-green-600 transition shadow-md"><svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}><path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" /></svg></button>
                                    </div>
                                </td>
                            </tr>
                        ))
  ...
  const TabLaporan = () => {
    ...
    // FUNGSI DOWNLOAD EXCEL (CSV) dengan Pemisah Titik Koma (;)
    const downloadCSV = () => {
        ...
        const rows = filteredReports.map(item => [
            ...
            `'${item.user?.no_telepon || "-"}'`, // Tanda kutip satu biar 0 di depan aman
            item.facility,
            `"${(item.purpose || '').replace(/"/g, '""')}"`, // Escape quote
            item.status
        ].join(";")); // GABUNG DENGAN TITIK KOMA

        // Tambahkan BOM (\uFEFF) untuk UTF-8 recognition di Excel
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 animate-fade-in min-h-[600px]">
        ...
        <div className="flex flex-col md:flex-row justify-between items-center mb-8 border-b border-gray-100 pb-6 gap-4 print:hidden">
            ...
            <div className="flex gap-3">
                <button 
                    onClick={downloadCSV}
                    className="bg-green-600 text-white px-5 py-2.5 rounded-lg text-sm font-bold flex items-center gap-2 hover:bg-green-700 transition shadow-md group"
                >
                ...
                <button 
                    onClick={handlePrint}
                    className="bg-[#2C3E50] text-white px-5 py-2.5 rounded-lg text-sm font-bold flex items-center gap-2 hover:bg-[#1a252f] transition shadow-md group"
                >
        ...
        {/* Filter Bar (Disembunyikan saat print) - DESIGN IMPROVED */}
        <div className="bg-gray-50 p-6 rounded-2xl border border-gray-200 mb-8 print:hidden shadow-sm">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div className="relative group">
                    ...
                    <div className="relative flex items-center transition-all duration-300 focus-within:shadow-md rounded-xl">
                        <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                            <svg className="w-5 h-5 text-gray-400 group-focus-within:text-[#008C9E] transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"></path></svg>
                        </div>
                        <input type="date" value={startDate} onChange={(e) => setStartDate(e.target.value)} className="w-full pl-10 pr-4 py-3 border border-gray-200 rounded-xl text-sm bg-white focus:outline-none focus:ring-2 focus:ring-[#008C9E]/50 focus:border-[#008C9E] transition-all duration-300 font-poppins shadow-sm" />
                    </div>
                </div>
                ...
                <div className="relative group">
                    ...
                    <div className="relative flex items-center transition-all duration-300 focus-within:shadow-md rounded-xl">
                        ...
                        <input type="date" value={endDate} onChange={(e) => setEndDate(e.target.value)} className="w-full pl-10 pr-4 py-3 border border-gray-200 rounded-xl text-sm bg-white focus:outline-none focus:ring-2 focus:ring-[#008C9E]/50 focus:border-[#008C9E] transition-all duration-300 font-poppins shadow-sm" />
                    </div>
                </div>
                ...
                <div className="relative group">
                    ...
                    <div className="relative flex items-center transition-all duration-300 focus-within:shadow-md rounded-xl">
                        <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                            <svg className="w-5 h-5 text-gray-400 group-focus-within:text-[#008C9E] transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
                        </div>
                        <select value={filterStatus} onChange={(e) => setFilterStatus(e.target.value)} className="w-full pl-10 pr-10 py-3 border border-gray-200 rounded-xl text-sm bg-white focus:outline-none focus:ring-2 focus:ring-[#008C9E]/50 focus:border-[#008C9E] transition-all duration-300 font-poppins shadow-sm appearance-none cursor-pointer">
                            <option value="Semua">Semua Status</option>
                            <option value="Selesai">Disetujui / Selesai</option>
                            <option value="Ditolak">Ditolak</option>
                        </select>
                        <div className="absolute inset-y-0 right-0 flex items-center px-4 pointer-events-none">
                            <svg className="w-4 h-4 text-gray-400 group-focus-within:text-[#008C9E] transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" /></svg>
                        </div>
                    </div>
                </div>
            </div>
  ...
  const TabManajemenKonten = () => {
    ...
    // State Footer (Ditambah jam_operasional)
    const [footerInfo, setFooterInfo] = useState({ 
        alamat: '', 
        email: '', 
        telepon: '', 
        jam_operasional: '', // Pastikan field ini ada
        instagram: '', 
        deskripsi: '' 
    });
    ...
    const handleUploadGallery = async (e) => {
        ...
        formData.append('image', selectedFile); // Kunci 'image' harus sama dengan validasi backend
        ...
        try { 
            ...
            alert("Gagal upload foto. Pastikan file < 5MB."); 
            console.error(error); 
        } finally { 
        ...
        <div className="space-y-8 animate-fade-in pb-10">
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
                <div className="p-6 border-b border-gray-100 bg-gray-50 flex justify-between items-center">
                    <div>
                        <h3 className="font-montserrat font-bold text-lg text-gray-800">Galeri Kegiatan</h3>
                        <p className="text-xs text-gray-500 mt-1">Upload foto kegiatan terbaru.</p>
                    </div>
                ...
                <div className="p-6">
                    <div className="mb-8 bg-blue-50/50 border-2 border-dashed border-blue-200 rounded-xl p-6 flex flex-col md:flex-row items-center gap-6">
                        ...
                        {previewUrl && (
                            <div className="w-24 h-24 rounded-lg overflow-hidden border border-gray-200 shadow-sm shrink-0">
                        ...
                        <button 
                            ...
                            className={`px-6 py-3 rounded-xl font-bold text-sm shadow-md transition-all flex items-center gap-2 ${uploadLoading || !selectedFile ? 'bg-gray-300 text-gray-500 cursor-not-allowed' : 'bg-[#008C9E] text-white hover:bg-[#007382] hover:shadow-lg active:scale-95'}`}
                        >
    ...
    <div className="min-h-screen bg-[#F8F9FD] font-poppins relative">
      <div className="container mx-auto px-4 md:px-12 lg:px-20 py-10">
        <div className="flex flex-col md:flex-row justify-between items-end mb-8 gap-4 print:hidden">
            <div><h1 className="text-2xl font-bold text-rptra-dark">Admin Dashboard</h1><p className="text-sm text-gray-500">Selamat datang, Administrator!</p></div>
            <div className="text-right hidden md:block"><p className="text-xs text-green-600 font-bold">● System Online</p></div>
        </div>
                ...
                <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden sticky top-8">
                    <div className="p-5 bg-[#2C3E50]">
                        <div className="flex flex-col items-center gap-2 text-center">
                            <div className="w-16 h-16 bg-white rounded-full p-1 shadow-sm"><img src="/images/logo.jpg" className="w-full h-full object-contain rounded-full" alt="Admin"/></div>
                            <div className="text-white"><p className="font-bold text-sm">ADMINISTRATOR</p><p className="text-[10px] opacity-70">RPTRA Lenteng Agung</p></div>
                        </div>
                    </div>
